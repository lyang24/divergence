...
All reads are 4KB adjacency pages. io_bytes/q = phy_reads/q × 4 KB.

### Per-query cold (pool.clear() each query — pure layout signal)

| Layout     | recall | p50 ms | p99 ms | QPS   | mis/q | phy_reads/q | hit%  |
|------------|-------:|-------:|-------:|------:|------:|------------:|------:|
...
All perq-cold unless noted. Adj reads = 4 KB/op, refine reads = 3 KB/op (768d × f32).

### Refine R sweep (perq-cold)

| Layout     | R   | recall | p50 ms | p99 ms | QPS   | adj_reads/q | io_ops/q | io_KB/q |
|------------|----:|-------:|-------:|-------:|------:|------------:|---------:|--------:|
| BFS        |  80 | 0.785  |   9.0  |  10.7  |  99.6 |       135.8 |    215.8 |     783 |
| heavy_edge |  80 | 0.785  |   7.8  |   9.7  | 112.1 |       108.3 |    188.3 |     673 |
| BFS        | 120 | 0.954  |   9.4  |  11.2  |  94.7 |       135.9 |    255.9 |     904 |
| heavy_edge | 120 | 0.954  |   8.3  |  10.2  | 106.3 |       108.2 |    228.2 |     793 |
| **BFS**    | **160** | **0.963** | **9.9** | **11.8** | **90.8** | **135.9** | **295.9** | **1024** |
| **heavy_edge** | **160** | **0.963** | **8.6** | **10.7** | **102.2** | **108.3** | **268.3** | **913** |
| BFS        | 200 | 0.964  |  10.5  |  12.4  |  86.1 |       135.9 |    335.9 |    1144 |
| heavy_edge | 200 | 0.964  |   9.2  |  11.2  |  96.7 |       108.3 |    308.3 |    1033 |

**At iso-recall 0.963 (R=160)**: heavy_edge is **13% faster** (p50 8.6 vs 9.9ms),
**+13% QPS** (102.2 vs 90.8), **20% fewer adj reads** (108.3 vs 135.9), **9% fewer IO ops**
(268.3 vs 295.9), **11% fewer IO bytes** (913 vs 1024 KB).

### Warm (R=160)

| Layout     | recall | p50 ms | p99 ms | QPS   | adj_reads/q | io_ops/q | io_KB/q |
|------------|-------:|-------:|-------:|------:|------------:|---------:|--------:|
| BFS        | 0.963  |   9.1  |  11.0  | 110.7 |       112.5 |    272.6 |     930 |
| heavy_edge | 0.963  |   8.2  |   9.9  | 122.0 |        87.4 |    247.4 |     830 |
| **delta**  |   —    | **-10%** | **-10%** | **+10%** | **-22%** | **-9%** | **-11%** |

---

## 5. SAQ + FP16 Disk Refine (perq-cold)

Adj reads = 4 KB/op, refine reads = 1.5 KB/op (768d × f16).

| Layout     | R   | recall | p50 ms | p99 ms | QPS  | adj_reads/q | io_ops/q | io_KB/q |
|------------|----:|-------:|-------:|-------:|-----:|------------:|---------:|--------:|
| BFS        | 160 | 0.963  |  11.4  |  13.4  | 80.8 |       135.9 |    295.9 |     784 |
| heavy_edge | 160 | 0.963  |  10.2  |  12.2  | 88.1 |       108.3 |    268.3 |     673 |
| **delta**  |     |   —    | **-11%** | **-9%** | **+9%** | **-20%** | **-9%** | **-14%** |

---

## 6. SAQ + Int8 Disk Refine (perq-cold)

Adj reads = 4 KB/op, refine reads = 0.75 KB/op (768d × i8).

| Layout     | R   | recall | p50 ms | p99 ms | QPS  | adj_reads/q | io_ops/q | io_KB/q |
|------------|----:|-------:|-------:|-------:|-----:|------------:|---------:|--------:|
| BFS        | 160 | 0.929  |  11.1  |  13.0  | 81.7 |       135.9 |    295.9 |     664 |
| heavy_edge | 160 | 0.929  |  10.1  |  12.5  | 88.8 |       108.3 |    268.3 |     553 |
| **delta**  |     |   —    | **-9%** | **-4%** | **+9%** | **-20%** | **-9%** | **-17%** |
...
Adj reads = 4 KB/op, refine reads = 3 KB/op (768d × f32).

| Layout     | Pinned | recall | p50 ms | p99 ms | QPS   | mis/q | io_ops/q | io_KB/q |
|------------|-------:|-------:|-------:|-------:|------:|------:|---------:|--------:|
| BFS        |     64 | 0.963  |   9.3  |  11.3  |  96.5 |  21.7 |    282.9 |     972 |
| heavy_edge |     64 | 0.963  |   8.4  |  10.1  | 105.3 |  17.4 |    254.2 |     857 |
| BFS        |    128 | 0.963  |   9.0  |  11.4  |  98.7 |  20.8 |    277.7 |     951 |
| heavy_edge |    128 | 0.963  |   8.2  |   9.9  | 107.1 |  16.4 |    249.4 |     838 |
...
| Config          | recall | p50 ms | p99 ms | QPS   | mis/q | phy_reads/q | hit%  |
|-----------------|-------:|-------:|-------:|------:|------:|------------:|------:|
...
| Config   | p50 ms | QPS   | phy_reads/q | hit%  |
|----------|-------:|------:|------------:|------:|
...
2. **The win is from fewer physical page reads**: adj phy_reads/q drops from ~135 to
   ~108 (perq-cold, -20%) and ~113 to ~87 (warm, -23%). Graph structure is identical —
   only page assignment changed. Better co-location means fewer unique pages touched
   per query, which translates directly to fewer NVMe reads.
...
4. **Refine IO dilutes the advantage**: adj reads are ~40% of total IO ops in SAQ+R160
   (108/268 for HE). As refine R increases, adj reads become a smaller fraction,
